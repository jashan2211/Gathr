...
// MARK: - Demo Event Factory
// Contains all individual event creation methods and supporting data pools

extension DemoDataService {

    // MARK: - Hosted Event 1: Maya's 25th Birthday Bash

    func createBirthdayBash(hostId: UUID, modelContext: ModelContext) -> Event {
        let event = Event(
        ...
        for (name, email, status, role, plusOnes) in birthdayGuests {
            ...
            for function in event.functions {
                ...
                if rand < 5 {
                    ...
                    invite.sentVia = [InviteChannel.whatsapp, .email, .sms].randomElement() ?? .email
                }
    ...
    func createGameNight(hostId: UUID) -> Event {
        let event = Event(
    ...
    func createFriendsgiving(hostId: UUID, modelContext: ModelContext) -> Event {
        let event = Event(
    ...
    func createRoadTrip(hostId: UUID, modelContext: ModelContext) -> Event {
        let event = Event(
    ...
    func createEngagementParty(currentUserId: UUID) -> Event {
    ...
    func createDemoDay(currentUserId: UUID) -> Event {
        let otherHostId = UUID()
    ...
    func createBeachBonfire(currentUserId: UUID) -> Event {
        let otherHostId = UUID()
    ...
    func createYogaInThePark() -> Event {
        let otherHostId = UUID()
        let event = Event(
    ...
    func createVinylSwapMeet() -> Event {
        let otherHostId = UUID()
        let event = Event(
    ...
    func generateRandomEvent(index: Int, hostId: UUID) -> Event {
        let category = massiveCategories.randomElement() ?? .party
        let location = massiveLocations.randomElement() ?? massiveLocations[0]
        let templates = massiveTemplates[category] ?? [EventTemplate(title: "Event", desc: "An event")]
        ...
        let durationHours = Double([2, 3, 4, 5, 6, 8, 12].randomElement() ?? 4)
        ...
        if category != .meetup && Bool.random() {
            ...
            for fname in funcs {
                let f = EventFunction(
                    ...
                    dressCode: [DressCode.casual, .smartCasual, .cocktail, .formal].randomElement() ?? .casual,
                    eventId: event.id
                )
        ...
        if isPublic && Bool.random() {
            ...
            let ga = TicketTier(name: "General Admission", price: prices.randomElement() ?? 0, capacity: capacity - 20, eventId: event.id)
            ga.soldCount = Int.random(in: 10...max(11, capacity / 2))
            event.ticketTiers = [ga]
            if Bool.random() {
                let vip = TicketTier(name: "VIP", price: (prices.randomElement() ?? 15) + 20, capacity: 20, eventId: event.id)
    ...
    func generateRandomBudget(for event: Event, modelContext: ModelContext) {
        let total = Double([500, 800, 1000, 1500, 2000, 3000, 5000].randomElement() ?? 1000)
        let budget = Budget(eventId: event.id, totalBudget: total)
    ...
    func randomFeatures(for category: EventCategory) -> Set<EventFeature> {
    ...
    var massiveCategories: [EventCategory] {
        [.party, .party, .meetup, .meetup, .conference, .concert, .wedding, .office, .custom]
    }

    var massiveLocations: [LocationData] {
        [
    ...
    var massiveTemplates: [EventCategory: [EventTemplate]] {
        [
            .party: [
    ...
    var massiveFunctionNames: [EventCategory: [String]] {
        [
